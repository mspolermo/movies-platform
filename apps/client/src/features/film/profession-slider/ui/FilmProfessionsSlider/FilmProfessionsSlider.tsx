...
import styles from './FilmProfessionsSlider.module.scss';

interface FilmProfessionsSliderProps {
  professions: TProfessionBased[];
  activeProfessionName: string | null;
  onProfessionChange: (professionName: string) => void;
}
...
export const FilmProfessionsSlider: React.FC<FilmProfessionsSliderProps> = ({
  professions,
  ...
  const scrollToActive = () => {
    if (!tabsRef.current || activeProfessionName === null) return;

    const activeIndex = professions.findIndex((p) => p.name === activeProfessionName);
  ...
  useEffect(() => {
    scrollToActive();
  }, [activeProfessionName, professions]);
      ...
      <div ref={tabsRef} className={styles.tabs}>
        {professions.map((profession) => (
          <button
            key={profession.id}
            type="button"
            className={`${styles.tab} ${activeProfessionName === profession.name ? styles.tabActive : ''}`}
            onClick={() => onProfessionChange(profession.name)}
            aria-label={`Выбрать профессию ${profession.name}`}
            aria-pressed={activeProfessionName === profession.name}
          >
