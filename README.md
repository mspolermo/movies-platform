...
После старта backend-микросервисы и базы поднимаются автоматически.

Для `kino-db` dev-данные теперь загружаются отдельным seeder-шагом после того, как `kino-db` создаст таблицы в Postgres через Sequelize.

Если нужно полностью пересоздать локальные dev-данные `kino-db`, пересоздай volume базы и подними стенд заново:
